...
- (void) dealloc {
    [self closeWithCode: PSWebSocketStatusCodeGoingAway reason: nil];
}
...
// Returns an open BLIPConnection to use to communicate with a given URL.
- (BLIPConnection*) socketToURL: (NSURL*)url error: (NSError**)outError {
    @synchronized(self) {
        BLIPConnection* socket = _sockets[url];
        if (!socket) {
            if (!_sockets) {
                // I'm closed already
                if (outError)
            ...
            socket = [[BLIPPocketSocketConnection alloc] initWithURL: url];
...
- (void) closeWithCode:(NSInteger)code reason:(NSString *)reason {
    ...
    for (NSURL* url in sockets) {
        BLIPPocketSocketConnection* socket = sockets[url];
        [socket closeWithCode: code reason: reason];
    }
}

- (void) close {
    [self closeWithCode: PSWebSocketStatusCodeNormal reason: nil];
}
